...
plt.plot(x_data[rev_trans_train_pred.shape[0] + 2*WINDOW_LENGTH:], df[DOWNLOAD_BITRATE_KEY][rev_trans_train_pred.shape[0] + 2*WINDOW_LENGTH:], label='ground truth')
plt.plot(x_data[rev_trans_train_pred.shape[0] + 2*WINDOW_LENGTH:], rev_trans_test_pred[:,0],  color='r', label='test prediction')
plt.show()
...
plt.show()
yerror = mean_absolute_percentage_error(df[DOWNLOAD_BITRATE_KEY][rev_trans_train_pred.shape[0] + 2*WINDOW_LENGTH:], rev_trans_test_pred[:,0])
yerror_mean = np.mean(yerror)
yerror_std = np.std(yerror)
